...
3. The hedging module— a continuous-action Gymnasium environment where a PPO agent learns how many shares to hold to neutralize option risk. The benchmark is Black-Scholes delta hedging. After 500k training timesteps, the PPO agent had a mean P&L of -0.37 compared to Black-Scholes's -2.15, but a standard deviation of 9.08 versus 1.24. The RL agent's higher mean is flattering but misleading— what matters for a hedger is variance, and by that measure Black-Scholes wins by a factor of seven. Ouch.
   ...
   **Update (Aug 2026):** a P&L attribution run found the mean story was stranger than "flattering but misleading." The agent's profile (−0.37, 9.08) is statistically indistinguishable from *not hedging at all* (−0.25, 8.94) — and refusing to hedge was the rational response to the reward I wrote. The environment pays no interest on short-stock proceeds, so hedging bleeds the risk-neutral drift uncompensated; that missing cash account is the *entire* −2.15 "cost" I'd attributed to discrete rebalancing. Add the financing term and the full BS hedge goes to (−0.29, 0.50) — free, with 18× less variance than the agent. The residual exposure regresses on terminal return (R² 0.61), not realized variance (R² 0.00): directional, not gamma. Full table and revised fix order in `experiments/ppo_hedging_vs_bs/RESULTS.md`; reproduce with `python -m experiments.ppo_hedging_vs_bs.attribution`. Twice now an agent's "failure" has been it optimizing exactly what I specified — the γ bug made the DQN too impatient, and the missing cash account paid PPO to sit on its hands. Price the objective before blaming the agent.
